...
INSERT INTO `health_report` VALUES (22,120,'5.110000133514404',4.59557,10.3,1,0,0,'2000-02-20','CL2');
...
INSERT INTO `login_credentials` VALUES ('CL1','CL1PASS');
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `general_update_trainer` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `general_update_trainer`(IN table_name_p VARCHAR(150), IN primary_key_p VARCHAR(150), 
                                        IN id_p VARCHAR(64), IN col_name_p VARCHAR(150), IN val_p VARCHAR(150))
BEGIN
        SET @r = 0;
        SET @test = CONCAT('SELECT EXISTS (SELECT * FROM ', table_name_p, ' WHERE ', primary_key_p, ' = ?)');
        PREPARE stmt FROM @test;
        EXECUTE stmt USING @id_p;
        SET @r := FOUND_ROWS();
        SELECT @r;
        DEALLOCATE PREPARE stmt;

        IF @r > 0 THEN
            SET @test = CONCAT('UPDATE ', table_name_p, ' SET ', col_name_p, ' = ? WHERE ', primary_key_p, ' = ?');
            PREPARE stmt FROM @test;
            EXECUTE stmt USING @val_p, @id_p;
            DEALLOCATE PREPARE stmt;
...
begin
	select b.week_day, b.mealtime, b.meal_description, b.calories from clients as a join diet_plan_description as b
    on a.plan_id = b.plan_id where a.client_id = client_id_p order by b.week_day, b.mealtime;
end ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `view_exercise_details_client`(in exercise_name_p varchar(64), in client_id_p varchar(10))
begin
	select c.exercise_name, c.exercise_description, e.equipment_name, e.equipment_description, e.equipment_id from clients as a
    join contain as b on a.workout_id = b.workout_id
    join exercise as c on b.exercise_id = c.exercise_id
    join used_in as d on d.exercise_id = b.exercise_id
    join equipment as e on d.equipment_id = e.equipment_id
    where client_id = client_id_p and c.exercise_name = exercise_name_p;
end ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
begin
	select b.week_day, c.exercise_name, b.sets, b.reps
    from clients as a join contain as b on a.workout_id = b.workout_id
    join exercise as c on b.exercise_id = c.exercise_id where a.client_id = client_id_p
    order by b.week_day;
end ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
begin
	select date, weight, height, bmi, fat_percentage, smoking, drinking, surgery  
    from health_report where client_id = client_id_p order by date desc;
end ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
begin
	select b.staff_name, b.contact_number, b.email, b.shift_start_time, b.shift_end_time, b.specialization, b.years_of_experience
    from clients as a join trainer as b on a.trainer_id = b.staff_id where a.client_id = client_id_p;
end ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2023-04-05 19:57:21
